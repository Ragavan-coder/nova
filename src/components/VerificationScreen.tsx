...
export default function VerificationScreen({ store }: { store: ReturnType<typeof useBlockchainStore> }) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col flex-1 h-full">
      {/* Search Bar */}
      {!searchedBatch && (
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 mb-8 max-w-3xl mx-auto w-full">
          <h2 className="text-2xl font-bold text-slate-800 mb-2 mt-2">Verify Vaccine Authenticity</h2>
          <p className="text-slate-500 mb-8">Enter the Batch ID to verify its journey on the blockchain ledger.</p>
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-slate-800 bg-slate-50 font-mono text-lg shadow-inner"
                value={searchQuery}
      ...
      {hasSearched && !searchedBatch && (
        <motion.div 
          ...
          className="bg-red-50 p-6 rounded-3xl border border-red-200 flex items-center gap-4 max-w-3xl mx-auto w-full"
        >
          <div className="p-3 bg-red-100 rounded-2xl">
            <AlertTriangle className="w-8 h-8 text-red-600" />
          </div>
          <div>
            <h3 className="text-red-800 font-bold text-xl uppercase tracking-tight">Invalid / Not Found</h3>
            <p className="text-red-600 font-medium mt-1">The requested Batch ID does not exist on the blockchain.</p>
          </div>
        </motion.div>
      )}
      ...
      {searchedBatch && (
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="grid grid-cols-1 md:grid-cols-12 gap-5 flex-1"
        >
          {/* Action Bar for Search */}
          <div className="col-span-full flex justify-between items-center mb-2">
            <button onClick={() => {setSearchedBatch(null); setSearchQuery('');}} className="text-blue-600 hover:text-blue-800 uppercase text-xs font-bold tracking-widest flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-xl transition-colors">
               Back to Search
            </button>
          </div>

          {/* Verification Result Card (Large) */}
          <div className="col-span-1 md:col-span-8 bg-white rounded-3xl p-6 md:p-8 border border-slate-100 shadow-sm flex flex-col">
            <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-4">
              <div>
                {searchedBatch.currentStatus === 'SAFE' ? (
                  <span className="px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full uppercase tracking-wider inline-flex items-center gap-1.5">
                    <ShieldCheck className="w-3.5 h-3.5" /> Verified on Blockchain
                  </span>
                ) : (
                  <span className="px-3 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full uppercase tracking-wider inline-flex items-center gap-1.5 border border-red-200">
                    <ShieldAlert className="w-3.5 h-3.5 animate-pulse" /> Alert: Tampered
                  </span>
                )}
                
                <h2 className="text-3xl font-bold mt-4 text-slate-900">{searchedBatch.name}</h2>
                <p className="text-slate-500 mt-1">Manufacturer: <span className="text-slate-900 font-semibold">{searchedBatch.manufacturer}</span></p>
              </div>
              <div className="md:text-right bg-slate-50 p-4 border border-slate-100 rounded-2xl">
                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Batch ID</p>
                <p className="text-xl font-mono text-blue-600 font-bold mt-1">#{searchedBatch.batchId}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mt-4">
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <p className="text-xs text-slate-500 uppercase mb-1 font-bold tracking-wider">Expiry Date</p>
                <p className="text-lg font-semibold text-slate-800">{new Date(searchedBatch.expiryDate).toLocaleDateString()}</p>
              </div>
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <p className="text-xs text-slate-500 uppercase mb-1 font-bold tracking-wider">Current Temp</p>
                {(() => {
                  const temp = searchedBatch.events[searchedBatch.events.length - 1].temperature;
                  const isSafe = temp >= 2 && temp <= 8;
                  return (
                    <p className={`text-lg font-semibold flex items-baseline gap-1 ${isSafe ? 'text-green-600' : 'text-red-600'}`}>
                      {temp.toFixed(1)}°C 
                      {isSafe ? <span className="text-xs text-slate-400 font-normal ml-1">(Optimal)</span> : <span className="text-[10px] uppercase font-bold bg-red-100 text-red-600 px-1.5 py-0.5 rounded ml-2 tracking-wider">Breach</span>}
                    </p>
                  );
                })()}
              </div>
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <p className="text-xs text-slate-500 uppercase mb-1 font-bold tracking-wider">Last Location</p>
                <p className="text-lg font-semibold text-slate-800 truncate" title={searchedBatch.events[searchedBatch.events.length - 1].location}>
                  {searchedBatch.events[searchedBatch.events.length - 1].location}
                </p>
              </div>
            </div>

            <div className="mt-auto pt-8 border-t border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-sm font-mono text-slate-600 truncate max-w-[200px] md:max-w-[300px]" title={searchedBatch.events[searchedBatch.events.length - 1].txId}>
          ...
          {/* Alert Panel (Warning) or Safe Status Panel */}
          {searchedBatch.currentStatus === 'TAMPERED' ? (
            <div className="col-span-1 md:col-span-4 bg-red-50 border border-red-100 rounded-3xl p-6 md:p-8 flex flex-col h-full shadow-sm">
              <div className="flex items-center gap-3 text-red-600 mb-6">
                <div className="bg-red-100 p-2 rounded-xl">
                    <AlertTriangle className="w-6 h-6 shrink-0" />
                </div>
                <h3 className="font-bold text-lg leading-tight uppercase tracking-wide">Tamper Alert</h3>
              </div>
              <p className="text-red-700 text-sm font-medium mb-5 leading-relaxed">
                Batch #{searchedBatch.batchId} flagged for attention. The integrity of this vaccine is compromised.
              </p>
              <div className="bg-white rounded-2xl p-5 border border-red-200 shadow-sm mt-auto">
            ...
            <div className="col-span-1 md:col-span-4 bg-blue-600 border border-blue-700 rounded-3xl p-6 md:p-8 flex flex-col h-full justify-between items-center text-center shadow-md relative overflow-hidden">
              ...
              <div className="relative z-10 w-full">
                <div className="w-20 h-20 bg-blue-500 text-white rounded-full flex items-center justify-center mx-auto mb-6 border-4 border-blue-400 shadow-inner">
                  <CheckCircle className="w-10 h-10" />
                </div>
                <h3 className="font-bold text-2xl text-white mb-2">Shipment Safe</h3>
                <p className="text-blue-100 text-sm font-medium">Transit parameters are within optimal range (2°C–8°C).</p>
              </div>
              <div className="w-full bg-blue-700/50 backdrop-blur-sm rounded-2xl p-5 border border-blue-500 shadow-sm mt-6 relative z-10">
          ...
          {/* Ledger Timeline (Bottom Wide) */}
          <div className="col-span-full bg-white rounded-3xl p-8 md:px-10 border border-slate-100 shadow-sm overflow-hidden mb-6">
                  ...
                  {searchedBatch.events.map((event, index) => {
                    ...
                    return (
                      <div key={event.txId} className="flex flex-col items-center w-32 relative">
                        {/* Connecting colored line progress */}
                        {index > 0 && (
